...
namespace KXFramework.Editor
{
    ...
    public class KXFrameworkEditor : EditorWindow
    {
        private const string LastScenePrefKey = "KXFramework.LastSceneOpen";

        private static KXFrameworkEditor Instance;

        private static string HelpText = string.Format(@"
KXFramework {0} <https://github.com/mr-kelly/KXFramework>
...
",KXFrameworkInfo.Version);

        [MenuItem("KEngine/KXFramework Options and Help")]
        private static void Open()
        {
            ...
            if (Instance == null)
            {
                Instance = GetWindow<KXFrameworkEditor>(true, "KXFramework Options");
            }
            Instance.Show();
        }
        ...
        {
            var luaPath = AppEngine.GetConfig("KXFramework.Lua", "LuaPath");
...
local UIBase = import('KXFramework/UIBase')

if not Cookie then local Cookie = Slua.GetClass('KXFramework.Cookie') end
...
if not Cookie then local Cookie = CS.KXFramework.Cookie end
if not UIModule then local UIModule = CS.KXFramework.UI.UIModule end
